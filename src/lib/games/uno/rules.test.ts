...
import { canPlayCard, createDeck, createGame, drawCard, getLegalPlays, playCard, callUno, rematch } from './rules.ts';
...
test('normal number cards hand the turn to the next player', () => {
  const state = stateWith([{ id: 'a', color: 'red', kind: 'number', value: 2 }, { id: 'b', color: 'blue', kind: 'number', value: 7 }], [{ id: 'c', color: 'green', kind: 'number', value: 5 }]);
  playCard(state, '0', 'a');
  assert.equal(state.currentPlayerIndex, 1);
  assert.equal(state.players[0].hand.length, 1);
});
test('skip skips exactly one player', () => {
  const state = stateWith([{ id: 'a', color: 'red', kind: 'skip' }, { id: 'b', color: 'red', kind: 'number', value: 2 }], [{ id: 'c', color: 'blue', kind: 'number', value: 3 }]);
  playCard(state, '0', 'a');
...
test('reverse changes direction and gives turn to the previous player', () => {
  ...
  state.players[0].hand = [{ id: 'a', color: 'red', kind: 'reverse' }, { id: 'd', color: 'red', kind: 'number', value: 2 }];
...
test('+2 makes the next player draw two and then passes the turn', () => {
  const state = stateWith([{ id: 'a', color: 'red', kind: 'draw2' }, { id: 'b', color: 'red', kind: 'number', value: 2 }], [{ id: 'c', color: 'blue', kind: 'number', value: 3 }]);
...
test('wild requires a chosen color and applies it', () => {
  const state = stateWith([{ id: 'a', color: 'wild', kind: 'wild' }, { id: 'b', color: 'red', kind: 'number', value: 2 }], [{ id: 'c', color: 'blue', kind: 'number', value: 3 }]);
...
test('wild draw four is illegal when the player has the current color', () => {
  const state = stateWith([{ id: 'a', color: 'wild', kind: 'wild4' }, { id: 'b', color: 'red', kind: 'number', value: 2 }], [{ id: 'c', color: 'blue', kind: 'number', value: 3 }]);
...
test('drawing is blocked while another player has an unresolved UNO call', () => {
  const state = stateWith([{ id: 'a', color: 'red', kind: 'number', value: 2 }, { id: 'b', color: 'red', kind: 'number', value: 3 }], [{ id: 'c', color: 'blue', kind: 'number', value: 4 }]);
...
test('playing the last card finishes the game with a winner', () => {
  const state = stateWith([{ id: 'a', color: 'red', kind: 'number', value: 2 }], [{ id: 'c', color: 'blue', kind: 'number', value: 4 }]);
  playCard(state, '0', 'a');
